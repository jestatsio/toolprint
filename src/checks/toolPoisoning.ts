...
function remediationFor(hit: PoisonHit): string {
  if (hit.id === "hidden-unicode") {
  ...
  return "Confirm this wording is intentional and benign; if not, stop using this server.";
}
  ...
  for (const source of textSources(capability)) {
    for (const hit of scanTextForPoisoning(source.text, source.where)) {
      ...
      findings.push({
        ...
        remediation: remediationFor(hit),
      });
    }
  }
